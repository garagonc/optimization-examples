...
def ComputeInitialStageCost_rule(model):
    return model.InitialStageCost==sum(model.weightGridExchange*model.Price_Forecast[t]*model.P_Grid_Output_ini[t]+model.weightEVAwayCharging*model.P_EV_ChargeAway_ini[t] for t in model.iniT)
model.InitialStageCost_Contraint = Constraint(rule=ComputeInitialStageCost_rule)

def ComputeFutureStageCost_rule(model):
    return model.FutureStageCost==sum(model.weightGridExchange*model.Price_Forecast[t]*model.P_Grid_Output_rec[t]+model.weightEVAwayCharging*model.P_EV_ChargeAway_rec[t] for t in model.recT)
