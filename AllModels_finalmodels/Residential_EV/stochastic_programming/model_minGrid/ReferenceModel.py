...
model.weightEVAwayCharging=Param(initialize=5.0)
...
model.P_EV_ChargeAway_ini=Var(model.iniT,within=NonNegativeReals)   #Optimization starts at home  
...
model.P_EV_ChargeAway_rec=Var(model.recT,within=NonNegativeReals)     
...
model.SoC_EV                     =Var(model.T_SoC,within=NonNegativeReals)
...
def _ini_ev_charging_away(model,t):
    return 0.0<=model.P_EV_ChargeAway_ini[t]<=(1-model.EV_ParkAtHome_Forecast[t])*model.EV_Max_Charge_Power
...
def _rec_ev_charging_away(model,t):
    return 0.0<=model.P_EV_ChargeAway_rec[t]<=(1-model.EV_ParkAtHome_Forecast[t])*model.EV_Max_Charge_Power
...
def evsoc(model,t):
    return model.SoC_EV[t]<=1.00
model.EVsoc=Constraint(model.T_SoC,rule=evsoc)

  
#############               State cost calculations              ##############
def ComputeInitialStageCost_rule(model):
    return model.InitialStageCost==sum(model.weightGridExchange*model.P_Grid_Output_ini[t]*model.P_Grid_Output_ini[t]+model.weightEVAwayCharging*model.P_EV_ChargeAway_ini[t]*model.P_EV_ChargeAway_ini[t] for t in model.iniT)
model.InitialStageCost_Contraint = Constraint(rule=ComputeInitialStageCost_rule)

def ComputeFutureStageCost_rule(model):
    return model.FutureStageCost==sum(model.weightGridExchange*model.P_Grid_Output_rec[t]*model.P_Grid_Output_rec[t]+model.weightEVAwayCharging*model.P_EV_ChargeAway_rec[t]*model.P_EV_ChargeAway_rec[t] for t in model.recT)
