...
class MaximizePV():
    ...
    def optimize_full_EM(self):
        ...
        ##################################       PARAMETERS            #################################
        ################################################################################################                                                      
        model.local_weight=Param(initialize=self.LocalTargetWeight)
        model.global_weight=Param(initialize=self.GlobalTargetWeight)
        ...
        model.ESS_Command=Param(model.T,initialize=self.DSO_Command)                        #DSO command for storage power
        ...
        ###########################################################################
        def obj_rule(model):  
            return sum(model.local_weight*(model.P_PV_Forecast[t]-model.P_PV_Output[t])*(model.P_PV_Forecast[t]-model.P_PV_Output[t])
                       +model.global_weight*(model.P_ESS_Output[t]-model.ESS_Command[t])*(model.P_ESS_Output[t]-model.ESS_Command[t])
                       for t in model.T)
