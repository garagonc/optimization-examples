...
class DynamicMinimizePExchange():
    ...
    def optimaldecisioncalculator(self,timestep,initialsoc):
        ...
        def objrule1(model):
            return  model.local_weight*model.P_GRID*model.P_GRID+model.global_weight*model.Deviation*model.Deviation+sum(model.Decision[x]*self.Value[timestep+1,initialsoc+x] for x in model.decision_ess)
...
if __name__=="__main__":
    ...
    dynprog_opt3=DynamicMinimizePExchange(scenario_24Ts,ess_soc_domain,decision_domain,ess_command,local_coef,global_coef,opt3)
