...
class DynamicMaximizePV():
    ...
    def optimaldecisioncalculator(self,timestep,initialsoc):
        ...
        def objrule1(model):
            return  model.local_weight*(self.P_PV_Forecast[timestep]*self.P_PV_Forecast[timestep]-model.P_PV*model.P_PV-model.Q_PV*model.Q_PV)+model.global_weight*model.Deviation*model.Deviation+sum(model.Decision[x]*self.Value[timestep+1,initialsoc+x] for x in model.decision_ess)
