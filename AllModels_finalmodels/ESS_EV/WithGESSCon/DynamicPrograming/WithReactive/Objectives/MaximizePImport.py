...
class DynamicMinimizePImport():
    ...
    def optimaldecisioncalculator(self,timestep,initialsoc):
        ...
        def objrule1(model):
            return  model.local_weight*model.P_GRID+model.global_weight*model.Deviation*model.Deviation+sum(model.Decision[x]*self.Value[timestep+1,initialsoc+x] for x in model.decision_ess)
