...
class MinimizePowerExchange():
    ...
    Objective: Minimization of power exchange with the grid)
    ...
    def build_abstract_model_withoutParameters(self):
        ...
        ###########################################################################
        def obj_rule(model):  
            return sum(model.local_weight[t]*(model.P_Grid_Output[t]*model.P_Grid_Output[t]+model.Q_Grid_Output[t]*model.Q_Grid_Output[t])
...
if __name__=="__main__":
    ...
    optimizationmodel=MinimizePowerExchange()
    ...
    with open('quadratic_MinimizePowerExchange.csv','w') as file:
        for ts in range(24):
