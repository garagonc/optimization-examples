...
class DynamicMinimizeGridExchange():
    ...
    def optimaldecisioncalculator(self,timestep,initialsoc):
        ...
        def objrule1(model):
            return model.P_GRID*model.P_GRID+model.Q_GRID*model.Q_GRID+sum(model.Decision[x]*self.Value[timestep+1,initialsoc+x] for x in model.decision_ess)
...
if __name__=="__main__":
    ...
    dynprog_opt3=DynamicMinimizeGridExchange(scenario_24Ts,ess_soc_domain,decision_domain,opt3)
    ...
    with open('dynamic_MinimizeGridExchange.csv','w') as file:
