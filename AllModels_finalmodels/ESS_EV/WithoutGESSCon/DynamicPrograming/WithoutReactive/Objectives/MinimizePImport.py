# -*- coding: utf-8 -*-
"""
Created on Tue Jun 26 12:12:34 2018
...
class DynamicMinimizePImport():
    ...
    def optimaldecisioncalculator(self,timestep,initialsoc):
        ...
        model.P_PV=Var(bounds=(0,self.P_PV_Forecast[timestep]),initialize=self.P_PV_Forecast[timestep])
        model.P_ESS=Var(bounds=(-self.ESS_Max_Charge_Power,self.ESS_Max_Discharge_Power))
        model.P_GRID=Var(bounds=(-self.P_Grid_Max_Export_Power,10000),initialize=0) 
        ...
        def objrule1(model):
            return model.P_GRID+sum(model.Decision[x]*self.Value[timestep+1,initialsoc+x] for x in model.decision_ess)
...
if __name__=="__main__":
    ...
    dynprog_opt3=DynamicMinimizePImport(scenario_24Ts,ess_soc_domain,decision_domain,opt3)
    ...
    with open('dynamic_MinimizePImp.csv','w') as file:
    ...
    value=dynprog_opt3.Value
    decision=dynprog_opt3.Decision